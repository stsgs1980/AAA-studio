...
const EMOJI_RE =
  /[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{2300}-\u{23FF}\u{2B50}\u{2B55}]/u;
...
const GRAPHIC_RE =
  /[\u{2500}-\u{257F}\u{2580}-\u{259F}\u{25A0}-\u{25FF}\u{2190}-\u{21FF}]/u;

// NOT blocked (level [I] or normal Unicode):
// U+2010-U+205E: General Punctuation (dashes, smart quotes, ellipsis) — info only
// U+00A0: Non-breaking space — common in text
// U+200B: Zero-width space — used in i18n

interface UnicodePolicyOptions {
  /** Block emoji (default: true) */
  emoji?: boolean;
  /** Block Unicode graphics: box drawing, arrows, shapes (default: true) */
  graphics?: boolean;
}

export const unicodePolicyRule: Rule.RuleModule = {
  meta: {
    type: "problem",
    docs: {
      description:
        "Enforce STD-DOC-003: block emoji and Unicode graphics in string literals",
    ...
    messages: {
      emoji:
        "Emoji '{{char}}' (U+{{hex}}) in string. Use text or lucide-react SVG icons instead. (STD-DOC-003)",
      graphic:
        "Unicode graphic '{{char}}' (U+{{hex}}) in string. Use ASCII (->, [WARNING]) or lucide-react SVG icons. (STD-DOC-003)",
    },
  },
  create(context) {
    const options: UnicodePolicyOptions = context.options[0] || {};
    const checkEmoji = options.emoji !== false;
    const checkGraphics = options.graphics !== false;

    if (!checkEmoji && !checkGraphics) return {};

    function scan(node: any, text: string) {
