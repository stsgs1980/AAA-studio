    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-text-primary">Skill Forge</h1>
        </div>
      ...
      {showNew && (
        <div className="flex items-end gap-2 rounded-xl border border-midnight-border bg-midnight-card p-3">
          <input value={newName} onChange={(e) => setNewName(e.target.value)}
            ...
            className="flex-1 h-9 px-3 rounded-lg border border-midnight-border bg-midnight-base text-sm text-text-primary placeholder:text-text-muted"
            autoFocus
            onKeyDown={(e) => { if (e.key === "Enter" && newName.trim()) createSkill(newName); }} />
          <button onClick={() => { if (newName.trim()) createSkill(newName); }}
            className="h-9 px-4 rounded-lg bg-brand-accent text-white text-sm font-medium hover:bg-brand-accent/90 disabled:opacity-40 disabled:pointer-events-none">Create</button>
          <button onClick={() => setShowNew(false)}
            className="h-9 px-3 rounded-lg border border-midnight-border text-text-secondary hover:bg-midnight-elevated text-sm">Cancel</button>
        </div>
      )}
