  ...
  return (
    <div className="space-y-3">
      ...
      <input value={query} onChange={(e) => setQuery(e.target.value)}
        ...
        className="w-full h-8 px-2 rounded-md border border-border bg-background text-sm text-foreground placeholder:text-muted-foreground" />
      ...
      {linkedIds.length > 0 && (
        <div className="space-y-1">
          ...
          {linkedIds.map((id) => {
            ...
            return (
              <div key={id} className="flex items-center justify-between px-2 py-1 rounded-md border border-border bg-brand-accent/5 text-sm">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-foreground">{std?.name ?? id}</span>
                  {std && <span className="text-[10px] px-1 py-0.5 rounded bg-muted text-muted-foreground">{std.category}</span>}
                </div>
                <button onClick={() => removeStandardId(id)} className="text-xs text-muted-foreground hover:text-brand-red transition-colors">Remove</button>
              </div>
            );
          })}
        </div>
      ...
      <div className="space-y-1">
        ...
        <div className="divide-y divide-border max-h-[40vh] overflow-y-auto">
          {filtered.filter((s) => !linkedIds.includes(s.id)).map((s) => (
            <div key={s.id} className="flex items-center justify-between px-2 py-1.5 text-sm hover:bg-muted rounded transition-colors">
