    ...
    return (
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="px-4 py-2 border-b border-border bg-background/50">
          <h2 className="text-sm font-semibold text-foreground">Standard Details</h2>
        </div>
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      <div className="px-4 py-2 border-b border-border bg-background/50 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-foreground truncate">{standard.name}</h2>
        {!editForm && (
      ...
      <div className="p-4 space-y-4">
        ...
        {editForm ? (
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            ...
            <span className="text-[11px] text-muted-foreground">v{standard.version}</span>
            <span className="text-[11px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{standard.category}</span>
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mb-3">
            <input value={newRule} onChange={(e) => setNewRule(e.target.value)}
              ...
              className="flex-1 h-8 px-3 rounded-md border border-border bg-card text-sm text-foreground placeholder:text-muted-foreground" />
          ...
          {standard.rules.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">No rules yet. Add one above.</p>
          ) : (
            <div className="divide-y divide-border rounded-lg border border-border overflow-hidden">
              {standard.rules.map((r) => (
                <div key={r.id} className="flex items-center gap-3 px-3 py-2.5 group hover:bg-background/30 transition-colors">
                  <GripVertical className="h-3.5 w-3.5 text-muted-foreground/30 shrink-0" />
                  <button onClick={() => toggleRule(r.id)}
                    className={cn("h-4 w-4 rounded border-2 transition-colors shrink-0",
                      r.enabled ? "bg-brand-accent border-brand-accent" : "border-border hover:border-text-muted")} />
