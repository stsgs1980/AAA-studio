...
export default function SettingsPage() {
  ...
  const [pc, setPc] = useState({
    ...
    apiKey: '', model: DEFAULT_LLM_SETTINGS.model,
  });
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      setPc({
        ...
        apiKey: data.llm_api_key ?? '', model: data.llm_model ?? DEFAULT_LLM_SETTINGS.model,
      });
      setLk((k) => k + 1);
  ...
  const handleSave = useCallback(async () => {
    try {
      await put({ ...settings, llm_provider: pc.providerId, llm_api_key: pc.apiKey, llm_model: pc.model });
      setSaved(true);
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      setPc({ providerId: DEFAULT_LLM_SETTINGS.providerId, apiKey: '', model: DEFAULT_LLM_SETTINGS.model });
      setLk((k) => k + 1);
    } catch { /* silent */ }
  }, [put]);
  ...
  if (loading) return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <PageSkeleton rows={3} /><PageSkeleton rows={3} />
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <LLMProviderCard key={lk} settings={settings} onSettingsChange={setSettings} onProviderChange={setPc} />
        <div className="rounded-xl border bg-card shadow-sm p-4">
          <h2 className="text-sm font-semibold mb-4">Appearance</h2>
          <div className="space-y-4">
            <F label="Theme"><select value={settings.theme ?? 'dark'} onChange={(e) => up('theme', e.target.value)} className={cls}>
              {['dark', 'light', 'system'].map((o) => <option key={o} value={o}>{o}</option>)}
            </select></F>
            <F label="Language"><select value={settings.language ?? 'en'} onChange={(e) => up('language', e.target.value)} className={cls}>
        ...
        <div className="rounded-xl border bg-card shadow-sm p-4">
          ...
          <div className="space-y-4">
            <F label="Max Tokens"><input type="number" value={settings.max_tokens ?? '4096'} onChange={(e) => up('max_tokens', e.target.value)} className={cls} /></F>
            <F label="Temperature">
        ...
        <div className="rounded-xl border bg-card shadow-sm p-4">
          ...
          <div className="space-y-4">
            <F label="Auto-save (ms)"><input type="number" value={settings.auto_save ?? ''} onChange={(e) => up('auto_save', e.target.value)} className={cls} /></F>
            <F label="Snap to Grid"><select value={settings.snap_to_grid ?? 'false'} onChange={(e) => up('snap_to_grid', e.target.value)} className={cls}>
