...
export default function SettingsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (data.llm_providers) {
        try { setProviders(JSON.parse(data.llm_providers)); } catch { /* */ }
      } else { setProviders(defaultProviders()); }
      setActiveId(data.llm_active_provider ?? DEFAULT_LLM_SETTINGS.activeProviderId);
      setActiveModel(data.llm_active_model ?? DEFAULT_LLM_SETTINGS.activeModel);
  ...
  const reset = useCallback(async () => {
    ...
    const defs = defaultProviders();
    const d: Record<string, string> = {
      theme: 'dark', language: 'en',
      llm_providers: JSON.stringify(defs),
    ...
    try {
      await put(d);
      setSettings(d); setProviders(defs);
      setActiveId(DEFAULT_LLM_SETTINGS.activeProviderId);
      setActiveModel(DEFAULT_LLM_SETTINGS.activeModel);
    } catch { /* */ }
  }, [put]);

  const up = (key: string, value: string) => setSettings(prev => ({ ...prev, [key]: value }));
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={reset} className="flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm hover:bg-accent">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <LLMProviderCard providers={providers} activeProviderId={activeId}
          activeModel={activeModel} onProvidersChange={setProviders}
          onActiveChange={(id, model) => { setActiveId(id); setActiveModel(model); }} />
