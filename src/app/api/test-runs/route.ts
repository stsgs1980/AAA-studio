...
export async function POST(request: Request) {
  try {
    ...
    if (!cases.length) throw BadRequest('No test cases found');
    ...
    return success({ runId: run.id, total: cases.length, passed, failed, errors, duration: Date.now() - start });
...
export async function GET() {
  try {
    const runs = await db.testRun.findMany({ orderBy: { createdAt: 'desc' }, take: 20 });
    return success(runs);
  } catch (error) { return handleError(error); }
}
