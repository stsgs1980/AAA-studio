...
export default function PromptStudioPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    await fetch(`/api/prompt-templates/${selected.id}`, {
      ...
      body: JSON.stringify({ content: editContent, variables: extractVars(editContent) }),
    });
    fetchTemplates();
  ...
  const handleCreate = useCallback(async () => {
    ...
    await fetch('/api/prompt-templates', {
      ...
      body: JSON.stringify({ name: newName.trim(), category: newCategory, content, variables: extractVars(content) }),
    });
    setNewName(''); setShowNew(false); fetchTemplates();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-[60vh]">
        <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
          <div className="px-3 py-2 border-b bg-muted/30">
            <h2 className="text-sm font-semibold">Templates ({filtered.length})</h2>
          </div>
          <div className="divide-y overflow-y-auto max-h-[55vh]">
            {filtered.map((t) => (
              <div key={t.id} className={cn('flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-accent/50 group', selected?.id === t.id && 'bg-accent')} onClick={() => setSelected(t)}>
                ...
                <button onClick={(e) => { e.stopPropagation(); handleDelete(t.id); }} className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-destructive/10 hover:text-destructive transition-all">
              ...
              <div className="space-y-3 h-full flex flex-col">
                <div className="flex gap-2 flex-wrap">
                  {selected.tags.map((tag) => (
                    <span key={tag} className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-muted"><Tag className="h-2.5 w-2.5" />{tag}</span>
                  ))}
                  {selected.variables.map((v) => (
                    <span key={v} className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary font-mono">{v}</span>
                  ))}
                </div>
                ...
                <div className="text-xs text-muted-foreground">
                  {selected.variables.length} variables &middot; {editContent.length} chars &middot; {editContent.split('\n').length} lines
                </div>
              </div>
            ) : (
