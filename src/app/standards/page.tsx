...
export default function StandardsManagerPage() {
  ...
  const fetchStandards = useCallback(async () => {
    try { setLoading(true); const res = await fetch('/api/standards'); if (!res.ok) throw new Error(); setStandards(await res.json()); }
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/standards', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: newName.trim(), severity: newSeverity }) });
  ...
  const toggleRule = useCallback(async (ruleIdx: number) => {
    ...
    try {
      const res = await fetch(`/api/standards/${selected.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rules: updated }) });
      if (!res.ok) throw new Error();
  ...
  if (loading) {
    return (
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-3"><Shield className="h-6 w-6 text-muted-foreground" /><h1 className="text-2xl font-bold tracking-tight">Standards Manager</h1></div>
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3"><Shield className="h-6 w-6 text-muted-foreground" /><h1 className="text-2xl font-bold tracking-tight">Standards Manager</h1></div>
        <button onClick={() => setShowNew(!showNew)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90"><Plus className="h-4 w-4" /> New Standard</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-[60vh]">
        <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
          <div className="px-3 py-2 border-b bg-muted/30"><h2 className="text-sm font-semibold">Standards ({standards.length})</h2></div>
          <div className="divide-y overflow-y-auto max-h-[55vh]">
            {standards.map((s) => { const Icon = SEV_ICON[s.severity] ?? Info; return (
              <div key={s.id} className={cn('flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-accent/50 group', selected?.id === s.id && 'bg-accent')} onClick={() => setSelected(s)}>
                ...
                <div className="flex-1 min-w-0"><div className="text-sm font-medium truncate">{s.name}</div><span className="text-[10px] text-muted-foreground">{s.rules.length} rules &middot; v{s.version}</span></div>
                <button onClick={(e) => { e.stopPropagation(); handleDelete(s.id); }} className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-destructive/10 hover:text-destructive transition-all"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
            ); })}
        ...
        <div className="lg:col-span-2 rounded-xl border bg-card shadow-sm overflow-hidden">
          <div className="px-4 py-2 border-b bg-muted/30"><h2 className="text-sm font-semibold">{selected ? selected.name : 'Select a standard'}</h2></div>
          <div className="p-4">
            {selected ? (
