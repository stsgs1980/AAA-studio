...
const FLOWS = [
  { name: 'Research Pipeline', description: 'Multi-step research workflow with summarization', status: 'active', nodes: JSON.stringify([{ id: 's1', type: 'start', position: { x: 250, y: 50 }, data: {} }, { id: 'i1', type: 'input', position: { x: 250, y: 150 }, data: { schema: { query: '' } } }, { id: 'l1', type: 'llm', position: { x: 250, y: 270 }, data: { systemPrompt: 'You are a research analyst.', temperature: 0.3 } }, { id: 'e1', type: 'end', position: { x: 250, y: 390 }, data: {} }]), edges: JSON.stringify([{ id: 'ea', source: 's1', target: 'i1', type: 'typed', data: { connectionType: 'command' } }, { id: 'eb', source: 'i1', target: 'l1', type: 'typed', data: { connectionType: 'sync' } }, { id: 'ec', source: 'l1', target: 'e1', type: 'typed', data: { connectionType: 'command' } }]) },
  { name: 'Code Review Flow', description: 'Automated code review with security check', status: 'active', nodes: JSON.stringify([{ id: 's1', type: 'start', position: { x: 200, y: 50 }, data: {} }, { id: 'l1', type: 'llm', position: { x: 100, y: 200 }, data: { systemPrompt: 'Review code for bugs.' } }, { id: 'l2', type: 'llm', position: { x: 350, y: 200 }, data: { systemPrompt: 'Check for security issues.' } }, { id: 'e1', type: 'end', position: { x: 200, y: 350 }, data: {} }]), edges: JSON.stringify([{ id: 'ea', source: 's1', target: 'l1', type: 'typed', data: { connectionType: 'delegate' } }, { id: 'eb', source: 's1', target: 'l2', type: 'typed', data: { connectionType: 'broadcast' } }, { id: 'ec', source: 'l1', target: 'e1', type: 'smoothstep' }, { id: 'ed', source: 'l2', target: 'e1', type: 'smoothstep' }]) },
  { name: 'Content Generation', description: 'Creative writing pipeline with review', status: 'draft', nodes: JSON.stringify([{ id: 's1', type: 'start', position: { x: 250, y: 50 }, data: {} }, { id: 'p1', type: 'prompt', position: { x: 250, y: 150 }, data: { template: 'Write a blog post about {{topic}}' } }, { id: 'l1', type: 'llm', position: { x: 250, y: 270 }, data: { systemPrompt: 'You are a creative writer.', temperature: 0.9 } }, { id: 'e1', type: 'end', position: { x: 250, y: 390 }, data: {} }]), edges: JSON.stringify([{ id: 'ea', source: 's1', target: 'p1', type: 'typed', data: { connectionType: 'command' } }, { id: 'eb', source: 'p1', target: 'l1', type: 'typed', data: { connectionType: 'sync' } }, { id: 'ec', source: 'l1', target: 'e1', type: 'smoothstep' }]) },
  { name: 'Data Analysis', description: 'Automated data processing and insight extraction', status: 'active', nodes: JSON.stringify([{ id: 's1', type: 'start', position: { x: 250, y: 50 }, data: {} }, { id: 'l1', type: 'llm', position: { x: 250, y: 180 }, data: { systemPrompt: 'Analyze the data.' } }, { id: 'o1', type: 'output', position: { x: 250, y: 310 }, data: {} }, { id: 'e1', type: 'end', position: { x: 250, y: 420 }, data: {} }]), edges: JSON.stringify([{ id: 'ea', source: 's1', target: 'l1', type: 'typed', data: { connectionType: 'command' } }, { id: 'eb', source: 'l1', target: 'o1', type: 'typed', data: { connectionType: 'sync' } }, { id: 'ec', source: 'o1', target: 'e1', type: 'smoothstep' }]) },
];
...
async function main() {
  ...
  // Create flows
  const createdFlows = await Promise.all(FLOWS.map((f) => db.flow.create({ data: f })));

  // Create initial version snapshots for each flow
  for (const flow of createdFlows) {
    await db.flowVersion.create({
      data: { flowId: flow.id, version: 1, nodes: flow.nodes, edges: flow.edges, description: 'Initial version' },
    });
  }
